...
function Switch({ className, ...props }: React.ComponentProps<typeof SwitchPrimitive.Root>) {
  return (
    <SwitchPrimitive.Root
      ...
      {...props}
    >
      <SwitchPrimitive.Thumb className="pointer-events-none block size-5 translate-x-0.5 rounded-full bg-[var(--cb-text)] shadow transition-transform data-[state=checked]:translate-x-5 data-[state=checked]:bg-[var(--cb-on-yellow)]" />
    </SwitchPrimitive.Root>
  );
}
