...
function AvatarFallback({ className, ...props }: React.ComponentProps<typeof AvatarPrimitive.Fallback>) {
  return (
    <AvatarPrimitive.Fallback
      className={cn("flex h-full w-full items-center justify-center rounded-full border border-[var(--cb-line)] bg-[var(--cb-surface-3)] text-[13px] font-bold uppercase text-[var(--cb-text-2)]", className)}
      {...props}
    />
  );
}
