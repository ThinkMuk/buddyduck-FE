...
describe("BuddyDuckApp screens", () => {
  ...
  it("renders CB-09 map preview, today schedule, and route actions", () => {
    ...
    expect(
      screen.getByRole("button", { name: "D-Day 일정 보기" }),
    ).toHaveAttribute("aria-pressed", "true");
    expect(
      screen.getByRole("button", { name: "D+1 일정 보기" }),
    ).toHaveAttribute("aria-pressed", "false");
    expect(screen.getByText("오늘 일정")).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "지도 핀 1: 잠실역 5번 출구" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "지도 핀 2: KSPO Dome 굿즈 라인" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "지도 핀 3: 잠실 카페 mood" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "지도 핀 공연: 공연 시작 (KSPO Dome)",
      }),
    ).toBeInTheDocument();
  ...
  it("switches CB-09 to D+1 schedule without keeping D-Day map route artifacts", () => {
    ...
    expect(
      screen.queryByText("D+1 일정은 아직 등록되지 않았어요"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "지도 핀 1: 잠실 숙소 체크아웃" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /일정 1.*잠실 숙소 체크아웃/ }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "지도 핀 1: 잠실역 5번 출구" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /일정 1.*잠실역 5번 출구/ }),
    ).not.toBeInTheDocument();
    expect(container.querySelector("polyline")).toHaveAttribute(
      "points",
      "28,48 50,44 68,60",
  ...
  it("selects the matching CB-09 schedule card when a map pin is clicked", () => {
    ...
    expect(
      screen.getByRole("button", { name: /일정 3.*잠실 카페 mood/ }),
  ...
  it("renders CB-10 search input, category chips, address candidates, and adds a place to the shared timetable", () => {
    ...
    expect(
      screen.getByText('주소 검색 결과 "송파대로 123"'),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole("button", { name: "식당" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    ...
    const toastRow = screen
      .getByText("테이크아웃 토스트 잠실점")
      .closest("[data-place-result]");
    expect(toastRow).not.toBeNull();

    fireEvent.click(
      within(toastRow as HTMLElement).getByRole("button", { name: "추가" }),
    );

    expect(
      useAppStore
        .getState()
        .timelineStopsByDay["d-day"].map((stop) => stop.place),
    ).toContain("테이크아웃 토스트 잠실점");
    expect(
      within(toastRow as HTMLElement).getByRole("button", { name: "추가됨" }),
    ).toBeDisabled();
  ...
  it("renders CB-11 block editor, updates steppers, and resets timetable changes", () => {
    ...
    expect(
      screen.getByText(
        "장소 블록 · 이동 블록 · 잠긴 공연 블록 · (수정 모드) 여유 시간",
      ),
    ).toBeInTheDocument();
    expect(screen.getByText("테이크아웃 토스트 잠실점")).toBeInTheDocument();

    const firstStop = screen
      .getByText("잠실역 5번 출구 (집합)")
      .closest("[data-timetable-stop]");
    expect(firstStop).not.toBeNull();
    fireEvent.click(
      within(firstStop as HTMLElement).getByRole("button", {
        name: "잠실역 5번 출구 머무는 시간 증가",
      }),
    );
    expect(
      within(firstStop as HTMLElement).getByText("25분"),
    ).toBeInTheDocument();

    const firstRoute = screen
      .getByText("도보 · 0.8km")
      .closest("[data-route-block]");
    expect(firstRoute).not.toBeNull();
    fireEvent.click(
      within(firstRoute as HTMLElement).getByRole("button", { name: "택시" }),
    );
    expect(
      within(firstRoute as HTMLElement).getByRole("button", { name: "택시" }),
    ).toHaveAttribute("aria-pressed", "true");
    ...
    expect(
      screen.queryByText("테이크아웃 토스트 잠실점"),
    ).not.toBeInTheDocument();
  ...
  it("reorders CB-11 timetable stops by dragging the grip handle", () => {
    ...
    const firstGrip = screen.getByRole("button", {
      name: "잠실역 5번 출구 순서 이동",
    });
    const thirdStop = screen
    ...
    expect(
      screen
        .getAllByTestId("timetable-place-name")
  ...
  it("renders CB-12 from the same shared timetable data", () => {
    ...
    const toastPin = screen.getByRole("button", {
  ...
  it("renders CB-11prime warning modal and returns to the editor without resetting edits", () => {
    ...
    expect(
      screen.getByText("＋ 장소 1개 추가됨 · 자동 역산 결과"),
    ).toBeInTheDocument();
    ...
    expect(
      within(dialog).getByText("사용 가능 시간 14:00 – 18:30 · 4h 30m"),
    ).toBeInTheDocument();
