...
re-read its `<data-source url="collection://...">` ID — don't assume the hardcoded ID
above is permanent.

**Important constraint**: this workspace doesn't have the Notion AI Enterprise plan, so
`notion-query-data-sources` (SQL or view mode) will fail with a `validation_error`
about upgrading. There's no way to deterministically list every row. Instead:
...
This is **semantic** search, not an exact filter — it ranks the entire database by
relevance to your query string, so it will return rows that have nothing to do with
your code mixed in with the ones that do. Treat every result as a _candidate_, never as
a confirmed match.

## 3. Verify each candidate with an exact match

For every candidate from step 2, `notion-fetch` the full page and read its `사용 화면`
property literally. This field is a comma-separated list, e.g.:
...
purely on semantic search's say-so — try a second query (the screen's English name from
...
that catches a wrong semantic-search match before it turns into a diff.
...
consumers — use `AskUserQuestion` to ask, in English, rather than guessing. (This
applies even though the rest of the conversation may be in Korean — phrase the
AskUserQuestion questions and options in English.)
